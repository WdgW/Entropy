...
class BuilderAIn extends BuilderAI{
    ...
    public void updateMovement(){
        ...
        if(following != null){
            ...
            if((retreatTimer += Time.delta) >= BuilderAI.retreatDelay || alwaysFlee){
                if(enemy != null){
                    unit.clearBuilding();
                    var core = unit.closestCore();
                    if(core != null && !unit.within(core, BuilderAI.retreatDst)){
                        moveTo(core, BuilderAI.retreatDst);
                        moving = true;
                    }
                }
            }
        ...
        if(unit.buildPlan() != null){
            ...
            if(assistFollowing != null && !hold){
                //对跟随半径进行调整
                moveTo(assistFollowing, assistFollowing.type.hitSize + unit.type.hitSize + 60f);
                moving = !unit.within(assistFollowing, assistFollowing.type.hitSize + unit.type.hitSize + 65f);
            }
            ...
            if(timer.get(timerTarget2, 20f)){
                ...
                Units.nearby(unit.team, unit.x, unit.y, BuilderAI.buildRadius, u -> {
                    if(found) return;
